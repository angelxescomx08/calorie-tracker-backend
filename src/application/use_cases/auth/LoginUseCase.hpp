...
class LoginUseCase : public std::enable_shared_from_this<LoginUseCase> {
...
public:
        ...
        std::function<void(std::string)>                   onError)
    {
        ...
        userRepo_->findByEmail(email,
            [self, password, onSuccessPtr, onErrorPtr]
            (std::optional<domain::User> userOpt) {
                if (!userOpt) { (*onErrorPtr)("Invalid email or password"); return; }
                domain::User user = *userOpt;

                self->userRepo_->getPasswordHash(user.id,
                    [self, user, password, onSuccessPtr, onErrorPtr](std::string storedHash) {
                        if (!self->hasher_->verify(password, storedHash)) {
                            (*onErrorPtr)("Invalid email or password");
                            return;
                        }
